...
INTERVAL=int(os.getenv('INTERVAL','1800')); MAX=int(os.getenv('MAX_SOURCE_CHARS','30000'))
...
def llm(prompt,model=None):
 ...
 try:
  body=json.dumps({'model':model,'messages':[{'role':'user','content':prompt}], 'temperature':0.1,'stream':False}).encode()
 ...
 except Exception as e: print('[llama.cpp]',e); return None
def make(d):
...
def process():
 ...
 if out.exists(): seen={hashlib.sha256(l.encode()).hexdigest() for l in out.read_text(encoding='utf-8').splitlines()}
 for p in RAW.glob('*.json'):
  try:
   ...
   if h not in seen:
    with out.open('a',encoding='utf-8') as f:f.write(line+'\n')
    seen.add(h)
def split():
