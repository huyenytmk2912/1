...
if need apt-get; then
  ...
  $SUDO dnf install -y python3 python3-pip curl git ca-certificates poppler-utils || $SUDO dnf install -y python3 python3-pip curl git ca-certificates
else
...
if [ -d "$APP_DIR/.git" ]; then
  ...
  echo "Existing non-git project found at $APP_DIR; keeping it. Remove it and rerun to reinstall cleanly."; exit 1
fi
...
MODEL="${MODEL:-}"
# Local AI is optional. On >=8 GiB RAM, use a small model unless explicitly disabled.
...
if [ -n "$MODEL" ]; then
  ...
  if ! need ollama; then
    if ! curl -fsSL https://ollama.com/install.sh | $SUDO sh; then
      echo "WARNING: Ollama installation failed; continuing in deterministic mode."; MODEL=""
    fi
  fi
  if [ -n "$MODEL" ]; then
    if ! pgrep -x ollama >/dev/null 2>&1; then nohup ollama serve >"$APP_DIR/data/logs/ollama.log" 2>&1 & sleep 4; fi
    if ! ollama pull "$MODEL"; then
      echo "WARNING: model pull failed; continuing without local AI."; MODEL=""
    fi
  fi
fi
...
case "${1:-status}" in
  status|build|readiness|export|verify|check-leakage|version|pipeline)
    exec "$APP_DIR/.venv/bin/python" "$APP_DIR/cli.py" "$1" ;;
  worker)
    exec "$APP_DIR/.venv/bin/python" "$APP_DIR/worker.py" ;;
  *) echo "Usage: $APP_DIR/run.sh {status|build|worker|verify|check-leakage|readiness|version|export|pipeline}"; exit 2;;
esac
EOF
